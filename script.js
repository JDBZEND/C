let main = {
  ...
  methods: {
    /*Configuración de juego*/gamesetup: function() {
      $('.gamecell').attr('chess', 'null');/*representa un movimiento de una pieza a su propia casilla.nulo no es necesario(turnos) */ 
    ...
    /*cordenadas del tablero*/
    /*Movimiento de seleccion de cada pieza*/
      let position = { x: '', y: '' };
      ...
      // estas opciones deben ser var en lugar de dejar
      var options = []; 
      var coordinates = [];
        ...
        case 'w_king': 
          if ($('#6_1').attr('chess') == 'null' && $('#7_1').attr('chess') == 'null' && main.variables.pieces['w_king'].moved == false && main.variables.pieces['w_rook2'].moved == false) {
            coordinates = [{ x: 1, y: 1 },{ x: 1, y: 0 },{ x: 1, y: -1 },{ x: 0, y: -1 },{ x: -1, y: -1 },{ x: -1, y: 0 },{ x: -1, y: 1 },{ x: 0, y: 1 },{x: 2, y: 0}].map(function(val){
             e (parseInt(position.x) + parseInt(val.x)) + '_' + (parseInt(position.y) + parseInt(val.y));
            });
          } else {
    ...
    w_options: function (position,coordinates) {
      ...
      coordinates = coordinates.map(function(val){ // convertir la x, y en coordenadas de identificación de cuadrícula reales;
          return (parseInt(position.x) + parseInt(val.x)) + '_' + (parseInt(position.y) + parseInt(val.y));
        }).filter(val => {
          let pos = { x: 0, y: 0 };
          pos.x = parseInt(val.split('_')[0]);
        ...
        }).filter(val => { // algoritmo para determinar las opciones de movimiento de la línea de visión para el alfil / torre / reina;
          if (flag == false){
    ...
    b_options: function (position,coordinates) {
      ...
      coordinates = coordinates.map(function(val){ // convertir la x, y en coordenadas de identificación de cuadrícula reales;
          ...
          if (!(pos.x < 1) && !(pos.x > 8) && !(pos.y < 1) && !(pos.y > 8)) { // si no está fuera de límites, devuelve la coordenada;
            return val;
          }
        }).filter(val => { // coordenadas de identificación de cuadrícula reales;
          if (flag == false){
            if ($('#' + val).attr('chess') == 'null'){
              return val;
            } else if (($('#' + val).attr('chess')).slice(0,1) == 'w') {
    ...
    capture: function (target) {
      let selectedpiece = {
        ...
        /*
        // alternar coordenadas resaltadas
      main.methods.togglehighlight (main.variables.highlight);
      main.variables.highlights.length = 0;
      // establece la pieza seleccionada en '' nuevamente
    ...
    endturn: function(){
      ...
      if (main.variables.turn == 'w') {
        ...
        // onfigurar la pieza seleccionada en ''
        main.variables.selectedpiece = '';
...
$(document).ready(function() {
  ...
  $('.gamecell').click(function(e) {
    ...
    } else if (main.variables.selectedpiece !='' && target.name == 'null') { // mover la pieza seleccionada
...
//Parte del fondo del juego con animación
var fondoJuego;
var juego = new Phaser.game(370,550,Phaser.CANVAS,'bloque_juego');
var estadoprincipal ={
  preload: function (){
    juego.load.image(img/tablero.jpg);
  },
  create: function(){
    juego.add.titleSprite(0,0,370,550,'fondo')},
    update: function(){
      fondoJuego.titlePosition.x -= 1;
    }
};
